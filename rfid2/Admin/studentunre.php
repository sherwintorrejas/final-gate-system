                ...
                <div class="card-body">
                                 ...
                                 <form method="post" enctype="multipart/form-data" action="registerstud.php">
                                    <div class="form-row">
                                        ...
                                        <input type="text" class="form-control" id="guardiannumber" name="guardiannumber" placeholder="exp:+6399xxxxxxxxx" required oninput="formatGuardianNumber(this)">
                                        </div>
                                    </div>
                                    <div class="form-row">
                                        <div class="form-group col-md-3">
                ...
                <div class="table-responsive p-3"  style="max-height: 400px; overflow-y: auto;">
                  ...
                  <form method="post">     
                    ...
                    <?php
                        ...
                        SELECT i.*
                        FROM information i
                        LEFT JOIN registudent r ON i.did = r.did
                        WHERE i.category = 'STUDENT' AND r.sid IS NULL
                    ";
                        $result = $conn->query($query);
                        
                        $sn = 0; // Serial number for listing
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                $sn++;
                                echo "<tr onclick='populateForm(\"" . htmlspecialchars($row['schoolid']) . "\", \"" . htmlspecialchars($row['name']) . "\", \"" . htmlspecialchars($row['department']) . "\", \"" . htmlspecialchars($row['category']) . "\")'>
                                        <td>".$sn."</td>
                                        <td>".$row['schoolid']."</td>
                                        <td>".$row['name']."</td>
                                        <td>".$row['department']."</td>
                                        <td>".$row['category']."</td>
                                        <td><a href='?action=delete&Id=".$row['did']."'><i class='fas fa-fw fa-trash'></i></a></td>
                                      </tr>";
                            }
                        } else {
    ...
    $(document).ready(function () {
                  ...
                  function populateForm(id, name, department, category) {
                      ...
                      document.getElementById('category').value = category;
                        ...
                        function formatGuardianNumber(input) {
