...
def send_sms_notification(guardiannumber, student_name):
    message = client.messages.create(
        body=f"Your child {student_name} has left ST. CECILIA'S COLLEGE-CEBU, INC.",
                    ...
                    if staff_result:
                        ...
                        if last_log:
                            ...
                            if last_category == 'OUT':
                                ...
                                print(f"Student ID {sid} ({student_name}) logged OUT.")
                                db.commit()
                                ser.write(b'SUCCESS_OUT\n')

                                # Send SMS notification only if the department does NOT start with 'BE' or 'BS'
                                if not department.lower().startswith(('be', 'bs')):
